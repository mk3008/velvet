create table velvet.run (
  run_id bigserial primary key

  , setting_id bigint not null
    references velvet.setting(setting_id)
...
comment on table velvet.run is
  '転送実行。転送実行時の引数、対象転送設定、実行全体の状態、実行時刻を記録するプロセスヘッダー。';

comment on column velvet.run.run_id is
  '転送実行ID。シーケンスで採番するサロゲートキー。';

comment on column velvet.run.setting_id is
  '転送設定ID。この転送実行が対象にする転送設定を参照する。';

comment on column velvet.run.run_arguments is
  '実行引数。転送実行開始時に指定された任意引数をJSONBで保持する。';

comment on column velvet.run.run_status is
  '実行状態。許可値は created, running, succeeded, failed, cancelled。実行全体のライフサイクル状態を表す。';

comment on column velvet.run.trigger_kind is
  '実行契機種別。日次、月次、手動、再実行などの実行契機の分類を保持する。';

comment on column velvet.run.snapshot_at is
  'スナップショット基準日時。転送元をどの時点の状態として扱ったかを説明する日時。';

comment on column velvet.run.started_at is
  '開始日時。転送実行の開始日時。';

comment on column velvet.run.finished_at is
  '終了日時。転送実行の終了日時。';

comment on column velvet.run.error_message is
  'エラーメッセージ。転送実行全体で発生したエラーの説明を保持する。';

comment on column velvet.run.created_at is
  '作成日時。レコード作成時刻。';

comment on column velvet.run.updated_at is
  '更新日時。レコード更新時刻。';

comment on column velvet.run.note is
  '備考。運用上の補足を記録する。';

comment on column velvet.run.execution_configuration is '集合phaseで使用したロック済みmaster設定の固定記録。完成SQL、hash、revisionを保持する。従来経路ではNULL。';
