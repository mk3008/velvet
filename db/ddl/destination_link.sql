create table rawsql_transfer.destination_link (
  ...
  generated_reassessment_sql_body text not null default '',
  ...
  note text null,
  constraint chk_setting_destination_execution_order_positive check (execution_order > 0),
  constraint chk_setting_destination_link_name_not_blank check (btrim (destination_link_name) <> ''),
  constraint chk_setting_destination_key_mapping_object check (jsonb_typeof (destination_key_mapping) = 'object'),
  constraint chk_setting_destination_mapping_definition_object check (jsonb_typeof (mapping_definition) = 'object'),
  constraint chk_setting_destination_diff_compare_excluded_columns_object check (
    diff_compare_excluded_columns is null
    or jsonb_typeof (diff_compare_excluded_columns) = 'object'
  ),
  constraint chk_setting_destination_generated_sql_status check (
    generated_sql_status in ('not_generated', 'success', 'failed')
  ),
