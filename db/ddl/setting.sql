...
comment on table velvet.setting is
  '転送設定。名前で一意に特定できる転送元データソース定義。基礎となる選択SQL、転送元キー定義、解析結果を管理する。転送指示、転送処理、転送結果ではない。';

comment on column velvet.setting.setting_id is
  '転送設定ID。サロゲートキー。';

comment on column velvet.setting.setting_name is
  '転送設定名。アプリケーションや後続処理から名前で参照するため一意にする。';

comment on column velvet.setting.description is
  '説明。転送設定の目的や業務上の意味を記録する。';

comment on column velvet.setting.source_sql_body is
  '基礎選択SQL本文。転送元データを抽出するSQLを保持する。SSSQL形式の任意検索条件を含めてよい。';

comment on column velvet.setting.source_sql_hash is
  '基礎選択SQL本文から算出したハッシュ。SQL本文の変更検知に使用する。';

comment on column velvet.setting.source_key_definition is
...
comment on column velvet.setting.source_sql_analysis_result is
  '基礎選択SQLの解析結果。戻り列、推定型などをJSONBで保持する。';

comment on column velvet.setting.search_condition_analysis_result is
  '検索条件解析結果。SSSQLから抽出した検索条件情報をJSONBで保持する。実行ロジックの正本ではなく、デバッグや検証補助に使用する。';

comment on column velvet.setting.source_sql_analysis_status is
  '基礎選択SQLの解析状態。許可値は not_analyzed, success, failed。';

comment on column velvet.setting.source_sql_analysis_error is
  '基礎選択SQLの解析エラー。解析失敗時の理由を記録する。';

comment on column velvet.setting.is_enabled is
  '有効フラグ。この転送設定を使用可能にするかを表す。';

comment on column velvet.setting.created_at is
  '作成日時。レコード作成時刻。';

comment on column velvet.setting.updated_at is
  '更新日時。レコード更新時刻。';

comment on column velvet.setting.note is
  '備考。実装・運用上の補足を記録する。';

comment on column velvet.setting.set_phase_definition is '集合phaseの明示opt-in。version、review revision、上限件数、転送元identityの完成SQLとSHA-256。NULLは従来経路。';
