create table rawsql_transfer.work_item (
  ...
  active_black_id bigint null references rawsql_transfer.active_black (active_black_id),
...
create index idx_work_item_run_destination on rawsql_transfer.work_item (run_id, destination_link_id, work_item_id);

create index idx_work_item_destination_source on rawsql_transfer.work_item (destination_link_id, source_key_hash);

create index idx_work_item_active_black on rawsql_transfer.work_item (active_black_id)
where
  active_black_id is not null;

comment on table rawsql_transfer.work_item is '転送作業対象。Dirty Keyを転送実行内で固定化し、転送元キー、転送モデル、転送操作の判断材料を保持する。';

comment on column rawsql_transfer.work_item.work_item_id is '転送作業対象ID。シーケンスで採番するサロゲートキー。';

comment on column rawsql_transfer.work_item.run_id is '転送実行ID。この作業対象を作成した転送実行を参照する。';

comment on column rawsql_transfer.work_item.dirty_key_id is 'Dirty Key ID。この作業対象の元になった変更検知履歴を参照する。';

comment on column rawsql_transfer.work_item.setting_id is '転送設定ID。この作業対象を評価した転送設定を参照する。';

comment on column rawsql_transfer.work_item.destination_link_id is 'Destination Link ID。この作業対象を評価した宛先別の転送単位を参照する。';
...
comment on column rawsql_transfer.work_item.source_key_json is '転送元キーJSON。この作業対象が扱う転送元キーをJSONBで保持する。';

comment on column rawsql_transfer.work_item.source_key_hash is '転送元キーハッシュ。転送元キーJSONの検索・比較を高速化するための補助値。';
...
comment on column rawsql_transfer.work_item.requires_black_insert_transfer is '黒伝追加転送要否。黒伝追加が必要かを表す。';

comment on column rawsql_transfer.work_item.requires_black_update_transfer is '黒伝更新転送要否。黒伝更新が必要かを表す。';

comment on column rawsql_transfer.work_item.requires_physical_delete_transfer is '物理削除転送要否。転送先行の物理削除が必要かを表す。';

comment on column rawsql_transfer.work_item.skip_reason is 'スキップ理由。転送操作を行わない理由を保持する。許可値は duplicate_ignore, no_op。';

comment on column rawsql_transfer.work_item.created_at is '作成日時。レコード作成時刻。';

comment on column rawsql_transfer.work_item.updated_at is '更新日時。レコード更新時刻。';

comment on column rawsql_transfer.work_item.note is '備考。運用上の補足を記録する。';
