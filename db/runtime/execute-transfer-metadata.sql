...
create or replace function velvet.record_skipped(fields jsonb) returns bigint
language plpgsql security invoker set search_path = pg_catalog as $body$
declare new_work bigint;
begin
  insert into velvet.work_item(
  ...
  insert into velvet.dirty_key_processing(
...
create or replace function velvet.record_black(fields jsonb, with_lineage boolean) returns void
language plpgsql security invoker set search_path = pg_catalog as $body$
begin
  insert into velvet.active_black(
...
create or replace function velvet.retire_active(fields jsonb, with_lineage boolean) returns void
language plpgsql security invoker set search_path = pg_catalog as $body$
declare retired integer;
begin
  update velvet.work_item
  set active_black_id = null
  where active_black_id = (fields->>'active')::bigint and destination_link_id = (fields->>'link')::bigint;
  delete from velvet.active_black
  where active_black_id = (fields->>'active')::bigint and destination_link_id = (fields->>'link')::bigint;
