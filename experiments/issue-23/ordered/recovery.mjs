...
export async function recover({db,ordered,measured,dirty,rtt}){
 ...
 const [producer,neighbor,other,monitor]=Array.from({length:4},()=>new Client({connectionString:url.toString()}));
 const report={backlog:10000,cap:1000,targetArrivalPerSecond:2,rtt,completed:false,timeline:[],otherRuns:[],neighbor:[],activity:[]};
 ...
 try{
  ...
  await db.query("set velvet.scale_fail='3'");await measured('outage',{maximum:1000,fail:true,latency:rtt});
  assert.equal(await pending(),10000);await db.query("set velvet.scale_fail=''");
  ...
  while(await pending()>5){
   ...
   const independent=(async()=>{const t=performance.now();const result=await ordered(otherClient,1000,'2');
    const elapsedMs=performance.now()-t;assert(elapsedMs<45000);report.otherRuns.push({elapsedMs,...result});})();
   const outcomes=await Promise.allSettled([measured('catch_up',{maximum:1000,latency:rtt}),independent]);
  ...
  report.catchupMs=performance.now()-start;report.arrivalsDuringCatchup=arrivals;report.remaining=await pending();
  ...
  for(let i=0;i<5;i++){await delay(1000);if(error)throw error;
   if(await pending())assert((await measured('steady',{maximum:1000,latency:rtt})).elapsedMs<45000);
   assert(await pending()<=5);}
  phase='correction';await db.query('update public.scale_source set amount=200');await dirty();
  assert((await measured('bounded_correction',{maximum:1000,latency:rtt})).elapsedMs<45000);
  if(error)throw error;report.completed=true;
 }finally{
  stopped=true;await Promise.allSettled(tasks);report.arrivals=arrivals;
  await Promise.allSettled([producer.end(),neighbor.end(),other.end(),monitor.end()]);
 }
