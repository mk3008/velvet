...
async function ordered(client,maximum){
 let runId, persisted=false;
 await client.query('begin');
 try{
  const [setting]=await query(client,q.settingSql,{id:'1'});
  const profileLinks=await query(client,q.linksSql,{id:'1'});
  ...
  [{run_id:runId}]=await query(client,q.runSql,{setting:'1',args:'{}'});
  await client.query('commit');persisted=true;await client.query('begin');
  assert.deepEqual((await query(client,q.settingSql,{id:'1'}))[0],setting);
  assert.deepEqual(await query(client,q.linksSql,{id:'1'}),profileLinks);
  const [result]=await query(client,sql`select * from public.scale_ordered_work(:run::bigint,1,:maximum::integer)`,{run:runId,maximum:maximum??null});
...
async function measured(scenario,{maximum,expected,fail=false,latency=rtt,loseCommit=false}={}){
 ...
 const row={scenario,elapsedMs,calls,parameterBytes:bytes,peak,cpu:process.cpuUsage(cpu),result};report.trials.push(row);console.log(JSON.stringify(row));
 if(mode==='ordered'){
...
await withFixture(async()=>{
 ...
 try{
  ...
  await db.query("set velvet.scale_fail='1'");await measured('failure',{fail:true,latency:0});assert.equal(await snapshot(),before);
  await db.query("set velvet.scale_fail=''");await measured('retry',{expected:5*links,latency:0});
  ...
  // Critical structural/recovery test: 10k backlog rather than the previous 600-key envelope.
  if(mode==='ordered'&&size===10000&&links===3){
   await setup(size,links);await configure();await dirty();
   let pending=size,arrivals=0,last=performance.now();const start=last;
   while(pending>5){
    const row=await measured('catch_up',{maximum:1000,latency:rtt});assert(row.elapsedMs<45000);
    const now=performance.now(),due=Math.floor((now-last)/500);last+=due*500;
    if(due){await query(db,sql`insert into rawsql_transfer.dirty_key(source_schema_name,source_table_name,source_key_json)
     select 'public','scale_source',jsonb_build_object('id',((i-1)%10000+1)::text) from generate_series(1,:due::int)i`,{due});arrivals+=due;}
    pending=(await db.query(`select count(*)::int n from rawsql_transfer.dirty_key d where
     exists(select 1 from rawsql_transfer.destination_link l where l.setting_id=1 and not exists(select 1 from rawsql_transfer.dirty_key_processing p
     where p.dirty_key_id=d.dirty_key_id and p.destination_link_id=l.destination_link_id))`)).rows[0].n;
    assert(performance.now()-start<180000,'finite catch-up evaluation ceiling exceeded');
   }
   report.recovery={elapsedMs:performance.now()-start,arrivals,pending,cap:1000,
    intake:'2/s accrued between Runs; producer is not concurrent; concurrency remains a separate gate'};
  }
