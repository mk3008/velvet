"""Summarize downloaded ordered-* artifacts; retain ranges, not only means."""
import json
import re
import statistics
import sys
...
output = []
for path in sorted(root.rglob('ordered-results.json')):
    report = json.loads(path.read_text())
    item = {k: report.get(k) for k in ['commit', 'mode', 'size', 'links', 'completed', 'differentialState']}
    item['routes'] = {}
    ...
    stats_path = path.with_name('ordered-postgres.jsonl')
    stats = []
