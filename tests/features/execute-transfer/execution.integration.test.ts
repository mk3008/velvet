...
describe.skipIf(!enabled).each(['row', 'routine'] as const)(
  'immutable Black Insert on PostgreSQL (%s)',
  (metadataMode) => {
    ...
    beforeAll(async () => {
      ...
      const order = JSON.parse(await readFile(new URL('order.json', root), 'utf8'))
        .order as string[];
    ...
    test('Run is committed before destination work and a deferred FK failure retains failed Run', async () => {
      ...
      const client = {
        async query(text: string, values?: unknown[]) {
          if (text.startsWith('insert into public.phase1_destination')) {
            expect(
              (await observer.query('select run_status from rawsql_transfer.run')).rows,
      ...
      try {
        ...
        expect(
          (await db.query('select run_id, run_status, error_message from rawsql_transfer.run'))
            .rows,
    ...
    test('failure recording errors remain secondary to the transfer error', async () => {
      ...
      const client = {
        async query(text: string, values?: unknown[]) {
          ...
          if (
            text.startsWith('update rawsql_transfer.run') &&
            text.includes("run_status = 'failed'")
          )
            throw recovery;
          return db.query(text, values);
        },
      ...
      expect((await db.query('select run_id, run_status from rawsql_transfer.run')).rows).toEqual([
