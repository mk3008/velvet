...
import type { QuerySource } from '../../../src/features/_shared/query-executor.js';
...
test('keeps repeated names, null and SQL-looking values separate from the statement', async () => {
  ...
  const query: QuerySource<{ value: string; optional: null }> = {
...
test('rejects unusable bindings before calling the driver', async () => {
  ...
  const query: QuerySource<{ value: unknown }> = {
