...
const statement = z
  .object({ text: z.string().min(1), sha256: z.string().regex(/^[a-f0-9]{64}$/) })
  .strict();
const common = { version: z.literal(1), revision: z.string().trim().min(1) };
const settingSchema = z
  .object({
    ...common,
    independentKeys: z.literal(true),
    maxDirtyKeys: z.number().int().positive().max(2147483647),
    sourceSchema: z.string().min(1),
    sourceTable: z.string().min(1),
    sourceSqlSha256: statement.shape.sha256,
    sourceIdentity: statement,
    dirtyIdentity: statement,
  })
  .strict();
const linkSchema = z.object({ ...common, evaluate: statement, black: statement }).strict();
const destinationSchema = z
  .object({ ...common, redProjection: statement, red: statement, verify: statement })
  .strict();
...
function columns(value: unknown): string[] {
  if (
    !Array.isArray(value) ||
    !value.length ||
    value.some((c) => typeof c !== 'string' || !/^[A-Za-z_][A-Za-z_0-9]*$/.test(c)) ||
...
export function loadSetPhase(setting: Master, links: Master[]) {
  ...
  accept(profile.sourceIdentity);
  accept(profile.dirtyIdentity);
  const configured = links.map((link) => {
    ...
    if (
      keys.some(
        (k) =>
          !allowed.includes(k) ||
          link.destination_columns.columns.find((c: Master) => c.name === k)?.type !== 'text',
      )
    )
    ...
    const excluded =
      link.diff_compare_excluded_columns === null
        ? []
        : link.diff_compare_excluded_columns?.columns;
    if (
      !Array.isArray(excluded) ||
      excluded.some((c: unknown) => typeof c !== 'string' || !allowed.includes(c))
    )
      throw new Error('Invalid set-phase comparison exclusions');
    accept(config.evaluate);
    accept(config.black);
    accept(destination.redProjection);
    accept(destination.red);
    accept(destination.verify);
...
export function materializeReviewed(
  slot: keyof typeof wrappers,
  sql: ReviewedStatement,
