...
function requireCount(
  ...
  if (typeof actual !== 'number' || actual !== expected)
    throw new Error(`${context} cardinality mismatch`);
}
...
export async function executeSetPhase(
  ...
  const query = (statement: Sql, params: Record<string, unknown> = {}) => {
    ...
    return client.query(b.text, b.values);
  };
  const materialize = (
    slot: Parameters<typeof materializeReviewed>[0],
    statement: ReviewedStatement,
    values: Record<string, unknown>,
  ) => {
    const b = materializeReviewed(slot, statement, values);
    return client.query(b.text, b.values);
  };
  ...
  if (Object.keys(arguments_).some((k) => k.startsWith('velvet_')))
    throw new Error('Reserved set-phase argument name');
  const args = { ...arguments_, velvet_run_id: run, velvet_setting_id: setting };
  try {
    const admitted = await query(q.admit, {
      setting,
      schema: config.profile.sourceSchema,
      table: config.profile.sourceTable,
      maximum: Math.min(maximum ?? config.profile.maxDirtyKeys, config.profile.maxDirtyKeys),
    });
    ...
    await check(q.identityCheck, {
      sourceRows: source.rowCount,
      keys: [...config.sourceKeys].sort(),
    });
    ...
    for (const c of config.configured) {
      ...
      await query(q.input, {
        link,
        mapping: JSON.stringify(c.link.mapping_definition.columns),
        keyMapping: JSON.stringify(c.link.destination_key_mapping.destinationKey),
      });
      await check(q.inputCheck, {
        mappedColumns: Object.values(c.link.mapping_definition.columns),
        sourceKeys: [...config.sourceKeys].sort(),
        link,
        keys: [...c.keys].sort(),
      });
      ...
      requireCount(
        (await query(q.work, { run, setting, link })).rowCount,
        decisions.rowCount,
        'Work',
      );
      for (const operation of ['red_insert', 'black_insert'] as const) {
        ...
        const sql = bindReviewed(
          operation === 'red_insert' ? c.destination.red : c.config.black,
          values,
        );
        ...
        if (operation === 'red_insert') {
          ...
          requireCount(
            (await query(q.retire, { link })).rowCount,
            targets.rowCount,
            'Active retirement',
          );
        } else {
          requireCount(
            (await query(q.active, { link })).rowCount,
            targets.rowCount,
            'Active insertion',
          );
        }
        requireCount(
          (await query(q.lineage, { run, setting, link, table: c.link.destination_table_name }))
            .rowCount,
          targets.rowCount,
          'Lineage',
        );
      ...
      requireCount(
        (await query(q.processing, { run, setting, link })).rowCount,
        decisions.rowCount,
        'Processing',
      );
    ...
    throw new Error(
      `Set-phase ${stage}: ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  }
}
