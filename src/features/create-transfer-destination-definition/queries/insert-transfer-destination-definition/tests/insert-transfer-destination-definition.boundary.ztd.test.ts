...
testZtd('create-transfer-destination-definition/insert-transfer-destination-definition boundary ZTD cases run through the fixed app-level harness', async () => {
  expect(cases.length).toBeGreaterThan(0);
  const evidence = await runQuerySpecZtdCases(cases, executeInsertTransferDestinationDefinitionQuery);
