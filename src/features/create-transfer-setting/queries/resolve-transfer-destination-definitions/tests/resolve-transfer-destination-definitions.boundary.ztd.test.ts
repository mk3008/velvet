...
testZtd('create-transfer-setting/resolve-transfer-destination-definitions boundary ZTD cases run through the fixed app-level harness', async () => {
  expect(cases.length).toBeGreaterThan(0);
  const evidence = await runQuerySpecZtdCases(cases, executeResolveTransferDestinationDefinitionsQuery);
